...
import { Prisma, PrismaClient } from "../../orm/generated/client";
  ...
  if (scoreSheetResults.length === 1) {
    ...
    try {
      ...
      scoreSheetResults.sort((a, b) => {
        const dateComp = a.timeslot.date.getTime() - b.timeslot.date.getTime()
        if (dateComp !== 0) return dateComp
        else return a.timeslot.court - b.timeslot.court
      }).sort((a, b) => {
        return a.timeslot.location > b.timeslot.location ? 1 : 0
      })

      const courtsLocations = groupBy(scoreSheetResults, game => game.timeslot.court + game.timeslot.location)

      for (const games of courtsLocations) {
        const sorted = games[1].sort((a, b) => a.timeslot.date.getTime() - b.timeslot.date.getTime())
