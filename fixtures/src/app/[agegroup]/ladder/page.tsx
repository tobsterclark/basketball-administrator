...
export default async function Page({
  ...
  const divisions = groupBy(teams, (team) => team.division).sort(
    ([divA], [divB]) => (divA || 10) - (divB || 10),
  );
      ...
      {divisions.map(([division, teams]) => (
        <div key={division} className="flex flex-col space-y-2">
          <p className="text-sm md:text-base font-semibold">
            {division && division > 0 ? `Div ${division}` : ``}
          </p>
          {LadderTable(teams)}
        </div>
      ))}
...
function LadderTable(teams: Team[]) {
  ...
  const orderedTeams = teams.sort((a, b) => {
    ...
    return comparison !== 0
      ? comparison
      : b.pointsPercentage() - a.pointsPercentage();
  });
  ...
  return (
    <table className="table-auto">
      ...
      <tbody>
        {orderedTeams.map((team, index) => LadderRow(team, index + 1))}
      </tbody>
      <caption className="text-gray-500 text-xs caption-bottom py-2">
        <p>
          Team positions are calculated by number of wins, draws, losses and
          forfeits,
        </p>
        <p>
          if teams have equal points, they are then compared by points
...
function LadderRow(team: Team, position: number) {
  return (
    <tr
      key={team.name}
      className="odd:bg-white text-sm text-gray-900 even:bg-gray-50"
    >
